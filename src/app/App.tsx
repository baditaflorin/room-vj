...
import { createRoomSession, type RoomSession } from '../features/session/roomSession'
import {
  emptyAudioFeatures,
  emptyPersonFrame,
  emptySurfaceFrame,
...
import { createRoomUrl, normalizeRoomCode, roomCodeFromUrl } from '../features/sync/roomLinks'
...
function AppView() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#08090d] text-white">
      <canvas ref={visualCanvasRef} className="absolute inset-0 h-full w-full" aria-label="Live shader canvas" />
      <video
        ref={videoRef}
        ...
        muted
      />
      <canvas ref={overlayCanvasRef} className="pointer-events-none absolute inset-0 h-full w-full mix-blend-screen" />
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3">
          <div>
            ...
            <p className="text-xs text-white/62">
              v{APP_VERSION} · build {APP_COMMIT} · main {commitQuery.data ?? APP_COMMIT}
            </p>
          </div>
      ...
      <section className="absolute inset-x-0 bottom-0 z-10 border-t border-white/10 bg-black/60 backdrop-blur-xl">
        <div className="mx-auto grid max-w-7xl gap-3 px-4 py-4 lg:grid-cols-[1.2fr_1fr_1.25fr]">
          ...
          <div className="grid grid-cols-[auto_1fr] items-center gap-3">
            <div className="flex items-center gap-2" role="group" aria-label="Palette">
          ...
          <div className="grid gap-2">
            <div className="flex gap-2">
              <label className="flex min-w-0 flex-1 items-center gap-2 rounded-md border border-white/15 bg-white/8 px-3">
                <LinkIcon size={16} aria-hidden="true" className="shrink-0 text-white/65" />
                <input
                  value={roomCode}
                  onChange={(event) => setRoomCode(normalizeRoomCode(event.currentTarget.value))}
      ...
      <aside className="absolute right-4 top-24 z-10 grid w-[min(22rem,calc(100vw-2rem))] gap-2 rounded-md border border-white/10 bg-black/45 p-3 text-sm text-white/80 backdrop-blur-xl">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="rounded-md bg-white/10 px-2 py-1 text-xs">{Math.round(status.fps)} FPS</span>
        </div>
        <div className="grid grid-cols-2 gap-2 text-xs">
          <Metric icon={<Camera size={14} />} label="Camera" value={status.camera} />
          <Metric icon={<Mic size={14} />} label="Mic" value={status.microphone} />
          ...
          <Metric label="Peers" value={`${status.sync.peerCount} · ${status.sync.status}`} />
        </div>
      </aside>
    </main>
  )
...
function paletteClass(name: PaletteName): string {
  if (name === 'thermal') return 'bg-[linear-gradient(135deg,#090a13,#f23b2f,#ffe28a)]'
  if (name === 'noir') return 'bg-[linear-gradient(135deg,#05070a,#9ee8ff,#ff4f99)]'
