...
function createMediaPipeTracker(landmarker: PoseLandmarkerInstance, fallback: PersonTracker): PersonTracker {
  let previous = emptyPersonFrame

  return {
    status: 'MediaPipe pose',
    detect(video, now) {
      if (video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA) return previous
      const result = landmarker.detectForVideo(video, now)
      const landmarks = result.landmarks[0]
      if (!landmarks || landmarks.length === 0) {
        const motionFrame = fallback.detect(video, now)
        previous = motionFrame.active ? motionFrame : { ...previous, active: false, confidence: 0, source: 'none' }
        return previous
      }

      const visible = landmarks.filter((point) => (point.visibility ?? 1) > 0.32)
      if (visible.length < 6) {
        const motionFrame = fallback.detect(video, now)
        previous = motionFrame.active ? motionFrame : { ...previous, active: false, confidence: 0, source: 'none' }
        return previous
      ...
      const radius = clamp(Math.max(maxX - minX, maxY - minY) * 0.68, 0.08, 0.34)
      const velocity = clamp(
        Math.hypot(centerX - previous.centerX, centerY - previous.centerY) * 18 + previous.velocity * 0.65,
      )
...
function createMotionTracker(): PersonTracker {
  ...
  return {
    ...
    detect(video) {
      if (!context || video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA) return previous
      ...
      lastFrame = new Uint8ClampedArray(image.data)
      if (changed < 16) {
        previous = { ...previous, active: false, confidence: 0, velocity: smooth(previous.velocity, 0, 0.2) }
        return previous
      }

      const centerX = clamp(((minX + maxX) / 2) / canvas.width)
      const centerY = clamp(((minY + maxY) / 2) / canvas.height)
      const radius = clamp(Math.max(maxX - minX, maxY - minY) / canvas.width, 0.07, 0.28)
      previous = {
        active: true,
