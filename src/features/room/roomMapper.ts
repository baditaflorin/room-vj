...
export function createRoomMapper(canvas: HTMLCanvasElement): RoomMapper {
  ...
  return {
    sample(video, demoTime = 0) {
      if (!sampleContext || !video || video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA) {
        previous = createDemoSurface(demoTime, previous)
        return previous
      }

      sampleContext.drawImage(video, 0, 0, SAMPLE_WIDTH, SAMPLE_HEIGHT)
      const image = sampleContext.getImageData(0, 0, SAMPLE_WIDTH, SAMPLE_HEIGHT)
        ...
        for (let gx = 0; gx < GRID_WIDTH; gx += 1) {
          ...
          for (let y = startY; y < endY; y += 2) {
            for (let x = startX; x < endX; x += 2) {
              ...
              const lum = (image.data[i] * 0.2126 + image.data[i + 1] * 0.7152 + image.data[i + 2] * 0.0722) / 255
              ...
              const rLum = (image.data[ri] + image.data[ri + 1] + image.data[ri + 2]) / 765
              const bLum = (image.data[bi] + image.data[bi + 1] + image.data[bi + 2]) / 765
              cellLum += lum
          ...
          const value = samples > 0 ? clamp(cellLum / samples + (cellEdge / samples) * 1.8) : 0
      ...
      const planeCount = grid.filter((value) => value > brightness + 0.16).length
      previous = {
    ...
    drawOverlay(surface, person) {
      ...
      if (person.active) {
        ...
        overlayContext.beginPath()
        overlayContext.arc(person.centerX * width, person.centerY * height, person.radius * width, 0, Math.PI * 2)
        overlayContext.stroke()
      }
    },
    resize,
...
function createDemoSurface(now: number, previous: SurfaceFrame): SurfaceFrame {
  const grid = new Array<number>(GRID_WIDTH * GRID_HEIGHT).fill(0).map((_, index) => {
