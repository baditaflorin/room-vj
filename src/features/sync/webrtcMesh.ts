...
export function createMeshSync(): MeshSync {
  ...
  const allPeerIds = () => Array.from(connections.keys()).filter((id) => id !== peerId)
  ...
  const connectToPeer = (targetPeerId: string) => {
    if (!peer || targetPeerId === peerId || connections.has(targetPeerId)) return
  ...
  const attachConnection = (connection: DataConnection) => {
    ...
    connection.on('open', () => {
      ...
      if (leader) {
        broadcastMessage({ type: 'peer-list', peers: [peerId, ...allPeerIds()] })
      }
    })
    connection.on('data', (raw) => {
      ...
      if (message.type === 'hello') {
        ...
        if (leader) broadcastMessage({ type: 'peer-list', peers: [peerId, ...allPeerIds()] })
      }
    ...
    connection.on('close', () => {
      ...
      status = connections.size > 0 ? `connected to ${connections.size} peer${connections.size === 1 ? '' : 's'}` : 'online'
    })
    connection.on('error', () => {
      connections.delete(connection.peer)
  ...
  return {
    async connect(nextRoomCode, mode) {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const timeout = window.setTimeout(() => reject(new Error('Peer signaling timed out')), 12000)
        peer.on('open', () => {
        ...
        peer.on('error', (error) => {
          window.clearTimeout(timeout)
          status = error.type === 'unavailable-id' ? 'room host already exists' : 'sync unavailable'
          reject(error)
        })
      })
    },
    broadcast(audio) {
      ...
      const hue = (audio.spectralCentroid * 0.45 + audio.beatCount * 0.013 + remoteHue * 0.08) % 1
      broadcastMessage({
        type: 'state',
        peerId,
        sentAt: Date.now(),
